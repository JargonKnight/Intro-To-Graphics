...
Version: 0.6 - * A Game-End screen will be implemented
             - * Sounds will be implemented to this version
                 making it the final version
...
class Plane(pygame.sprite.Sprite):
    def __init__(self):
        ...
        if not pygame.mixer:
            ...
            pygame.mixer.init()
            self.sndFire = pygame.mixer.Sound("Sounds/missileFire.ogg")
            self.sndCrash = pygame.mixer.Sound("Sounds/planeExplode.ogg")
            self.sndEngine = pygame.mixer.Sound("Sounds/planeSound.ogg")
...
class Explosion(pygame.sprite.Sprite):
    def __init__(self, posx, posy):
        ...
        pygame.mixer.init()
        self.sndExplode = pygame.mixer.Sound("Sounds/missileExplode.ogg")
...
def game():
        ...
        while keepGoing:
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.MOUSEBUTTONUP:
                    #if sound already playing then stop it and restart it
                    plane.sndFire.stop()
                    plane.sndFire.play()
            ...
            for missile in pygame.sprite.groupcollide(missileSprites, enemySprites, False,False):
                for enemy in pygame.sprite.groupcollide(enemySprites, missileSprites, False, False):
                    ...
                    #stop exploding sound from playing and start it again
                    explosion.sndExplode.stop()
                    explosion.sndExplode.play()
        ...
        return scoreboard.score, gameOver, donePlaying, plane, scoreboard, bgImage
...
def endGame(score, plane, scoreboard, bgImage):
    pygame.display.set_caption("Sky Defender")
...
def main():
    ...
    while not donePlaying:
        ...
        if not donePlaying:
            while not gameOver:
                score,gameOver,donePlaying, plane, scoreboard, background = game()

            donePlaying, gameOver = endGame(score, plane, scoreboard, background)
