...
class Frogger(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        #label class to show the name of the level
...
#fireball class is a fireball sprite that leapy can shoot at his enemy
#based on the direction of leapy, different images are loaded
#for the sprite
class Fireball(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        #kill the sprite if it goes of the screen
        if self.rect.left < 0:
            self.kill()
        if self.rect.top < 0:
            self.kill()
...
#flybomb class are sprites that the enemy boss shoots at leapy
...
def main():
    ...
    background = pygame.image.load("Resources/sewerBackground.jpg")
    ...
    while keepGoing:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                #if space key is pressed check a couple things
                if key[pygame.K_SPACE]:
                    #first if the counter has reached its limit so
                    #the player cannot spam the spacebar
                    if fireBallCounter > 15:
                        #then only fire a fireball if there are less
                        #than 3 on the screen already
                        if len(pygame.sprite.Group(allFireballs)) < 3:
                            fireball = Fireball(frogger.rect.centerx, frogger.rect.centery, frogger.direction)
                            allFireballs.add(fireball)
                            fireBallCounter = 0
        #increase the fireball counter each iteration
        #of the gameloop
        fireBallCounter += 1
        ...
        #checking bounds of the boss so he doesnt go off screen                            
        if boss.rect.left < 0:
            boss.changeDirection(frogger.rect.centerx, frogger.rect.centery)
        ...
        #change direction when the number is above 950
        if rand > 950:
            boss.changeDirection(frogger.rect.centerx, frogger.rect.centery)

        #if the counter is greater than 75, shoot another flybomb
        #this gives an interval for the boss to shoot flybombs
        if counter > 75:
            counter = 0
